...
import numpy as np
from matplotlib.colors import LinearSegmentedColormap, Normalize, hsv_to_rgb, rgb_to_hsv, to_rgb
...
class Plotting:
    ...
    def create_custom_cmap(self, max_scale, base_colors, adjustment_factors=None):
        ...
        adjusted_colors = [
            (pos, adjust_color_properties(color, adjustment_factors)) for pos, color in base_colors
        ]
    ...
    def plot_isolines(self, data, fig=None, axes=None, titles=None, show_colorbar=True, colorbar_kwargs=None):
        if data.ndim == 2:
        ...
        for i, ax in enumerate(axs):
            ...
            parallels = m.drawparallels(np.arange(30.0, 90.0, 30.0), labels=[1, 0, 0, 0], linewidth=0.3)
            m.drawmeridians(
                np.arange(-180.0, 181.0, 30.0), labels=[0, 0, 0, 0], linewidth=0.3
            )  # no meridian labels
            ...
            draw_custom_lon_labels(
                ...
                fontsize=8,
                fontname="serif",
                tick_degrees=[270, 300, 330, 0, 30, 60, 90],
            )
        ...
        if _own_axes and show_colorbar:
            ...
            cb = fig.colorbar(
                cf_handles[0], ax=axs, orientation="horizontal", fraction=0.04, pad=0.08, **colorbar_kwargs
            )
            cb.set_label("[hPa]")
    ...
    def plot_data(self, data, ncols=None):
        ...
        for i, ax in enumerate(self.axs):
            ...
            m.drawparallels(np.arange(30.0, 90.0, 30.0), labels=[1, 0, 0, 0])
            meridians = m.drawmeridians(np.arange(-180.0, 181.0, 30.0), labels=[0, 0, 0, 1])
        ...
        try:
            cbar = self.fig.colorbar(
                ax=self.axs.ravel().tolist(), orientation="horizontal", extend="both", pad=0.1, shrink=0.7
            )
