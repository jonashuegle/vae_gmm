...
def pca_kmeans_clusters(
    ...
    *,
    ...
    patterns = da.assign_coords(cluster=labels).groupby("cluster").mean("time").sortby("cluster")
...
class PatternComparator:
    ...
    def plot_comparison_grid(self, pat_mod, mapping, plotter, figsize=(13, 3.5 * 4), std_ref=1.0):
        ...
        gs = GridSpec(
            nC, 3, figure=fig, left=0.04, right=0.98, top=0.97, bottom=0.23, wspace=-0.15, hspace=0.1
        )
    ...
    def plot_taylor_best_match_single(
        ...
        curved_text(
            ...
            **{
                k: v
                for k, v in curved_text_kwargs.items()
                if k not in ("r_max_multiplyer", "start_angle", "end_angle")
            },
        )
        ax.text(
            r_max * 0.85,
