...
def visualize_tsne(
    ...
    tsne = TSNE(
        n_components=n_components, random_state=random_state, perplexity=perplexity, max_iter=max_iter
    )
    ...
    for i, cluster in enumerate(sorted_clusters):
        ...
        if n_components == 2:
            trace = go.Scatter(
                ...
                mode="markers",
                marker=dict(size=5, opacity=1, color=colors[i] if colors is not None else None),
                name=cluster_label,  # regime name, or the default
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            itemsizing="constant",  # keeps legend entry sizes consistent
        ),
    )
...
def visualize_tsne_matplotlib(
    ...
    cluster_names=None,  # list of names, e.g. ["NAO+", ...]
    cluster_colors=None,  # RGB colours, same order as cluster_names
    perplexity=30,
    random_state=42,
    fig=None,
    ax=None,
    ...
    **kwargs,
    ...
    if cluster_names is not None:
        # Order the names to match the actual cluster indices.
